...
class LoginCache{
	...
	public int getMaxUserLoginId(String user_id) {
		int max_login_id = 100;
        if (this.login_map.containsKey(user_id)){
			HashMap<String,WebLogin> temp = this.login_map.get(user_id);
			for (WebLogin login:temp.values()){
				if (login.getInternalId() > max_login_id) {
					max_login_id = login.getInternalId();
				}
			}
		}
		return max_login_id;
